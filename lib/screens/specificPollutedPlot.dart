...
class specificMyPlot extends StatelessWidget {
  specificMyPlot({ required this.option});
  final String option;
  @override
...
class _ImageFromAPIState extends State<ImageFromAPI> {
  ...
  Future<void> fetchImage() async {
    String imageUrl = 'http://192.168.1.11:8080/specificpollutedplot?query='+widget.option;
    ...
    return Stack(
      children: [
        CircularParticle(
          width: _w,
          height: h,
          particleColor:Colors.redAccent.withOpacity(0.2),
...
class _MyCustomLoadingWidgetState extends State<MyCustomLoadingWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Center(
        child: AvatarGlow(
          glowColor: Color(0xFFFF5757).withOpacity(0.5),
          endRadius: _w/2,
          ...
          child: Container(
            ...
            width: _w/2.5,
            decoration: BoxDecoration(
                color:  Color(0xFFFF5757), borderRadius: BorderRadius.circular(99)),
            child: Icon(
              Icons.auto_graph_outlined,
              color: Colors.white,
              size: 40,
